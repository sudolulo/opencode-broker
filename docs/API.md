...
| `/forget` | `{ sessionID, completed? }` | Drop the lease at the end of a turn. The assignment (the session's pinned model) stays and ages out after 14 days; `completed: true` also clears probation. Assignments are also capped at 512: eviction is oldest-first, but it spends every settled `oneShot` entry before any session pin, and never touches an assignment whose session still holds a live lease. |
