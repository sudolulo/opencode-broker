...
// reads and cache writes together, which is the size that has to fit the window. That total is
// also kept split into `input`, `cacheRead` and `cacheWrite`, always present even at zero:
// after a burn-watch stop the question is whether the session re-sent an uncached prompt, and
// `prompt` alone cannot answer it -- a `cacheRead: 0` is exactly the signal.
