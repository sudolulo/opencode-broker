...
// CRITICAL: THE DAEMON MUST NOT TRUST A PUBLISHER. Model admission (drop what this host cannot
// resolve) also runs in discoverSubscriptionTargets, but that code runs in the PUBLISHER --
// plugin/router.js, loaded into every opencode process at spawn and re-publishing on every
// chat.message. A pane started before an admission fix therefore keeps publishing the OLD,
// unfiltered inventory for its entire life, and re-poisons a freshly restarted daemon within
// seconds (measured 2026-09-23: `discovered: []` became three unresolvable gpt-6 models again,
// and every worker-tier lease died with ProviderModelNotFoundError). Repeating the filter at
// the ingest boundary is the only version of it a client cannot be behind.
//
// Same predicate as discovery: the MODEL_ID shape guard plus the resolver-view intersection.
// The resolver view is ALWAYS intersected here -- unlike publishSubscriptionInventory, an
// /inventory body is a client's claim, never the host's own provider list -- so a missing or
// empty snapshot admits NOTHING. That is fail-closed by design: every tier keeps its
// configured static pins, which this filter never touches.
//
// Expects an inventory that normalizeDiscoveredInventory has already shaped. Returns the
// filtered inventory plus what was dropped, so the caller can report it rather than swallow it.
export const admitResolvableInventory = (inventory, resolvableModels) => {
  const resolvable = resolvableModels instanceof Set
    ? resolvableModels
    : new Set(Array.isArray(resolvableModels) ? resolvableModels : []);
  const targets = {};
  const droppedTargets = [];
  for (const [id, target] of Object.entries(inventory?.targets ?? {})) {
    const failure = modelAdmissionFailure(target?.providerID, target?.modelID, resolvable);
    if (failure) {
      droppedTargets.push({ id, providerID: target?.providerID ?? "", modelID: String(target?.modelID ?? ""), reason: failure });
      continue;
    }
    targets[id] = target;
  }
  // The catalog side-tables are keyed `providerID/modelID` and must move WITH the model:
  // a window, output budget or variant list left behind for a model that can never be
  // leased outlives the target that justified it, and targetContext would still read it
  // for any later target carrying the same name.
  const droppedModelKeys = [];
  const admitKeys = (map) => {
    const kept = {};
    for (const [key, value] of Object.entries(map ?? {})) {
      if (!resolvable.has(key)) {
        droppedModelKeys.push(key);
        continue;
      }
      kept[key] = value;
    }
  ...
  return {
    inventory: {
      ...inventory,
      targets,
      modelContexts: admitKeys(inventory?.modelContexts),
      modelOutputs: admitKeys(inventory?.modelOutputs),
      modelVariants: admitKeys(inventory?.modelVariants),
    },
    droppedTargets,
    // De-duplicated: the same model key is normally dropped from more than one side-table.
    droppedModelKeys: [...new Set(droppedModelKeys)].sort(),
  };
};
...
const eligible = (target, { active = {}, circuits = {}, health = {}, localModels, now = Date.now(), targets = TARGETS, contextTokens = null, modelContexts = {}, modelOutputs = {}, budgets = {}, budgetConfig = PROVIDER_BUDGETS, ignoreContext = false, ignoreCapacity = false }) => {
  ...
  // ignoreCapacity answers a different question from selection: "would this target serve the
  // request if its slots were free?" The last-resort pass below uses it to tell a target that is
  // merely BUSY from one that genuinely cannot hold the request.
  if (!ignoreCapacity && targetFull(target, active)) return false;
...
export const chooseTarget = ({ profile, tier, active = {}, circuits = {}, health = {}, cursors = {}, localModels, now = Date.now(), targets = TARGETS, contextTokens = null, modelContexts = {}, modelOutputs = {}, budgets = {}, budgetConfig = PROVIDER_BUDGETS, deals = undefined, tierWeights = undefined, waitedMs = 0 }) => {
  ...
  // A rung with a configured delay is not a candidate for ORDINARY selection until the caller
  // has already waited that long for its primary. This is what keeps a bursty background profile
  // (memory) from grabbing a scarce shared slot the instant its own lane is busy, while leaving
  // an undelayed rung -- every existing profile -- selected immediately as before.
  // ☆ Filter HERE, at the call site, and nowhere else. `fallbackTargetGroupsFor()` and
  // `targetEligibleIDsFor()` must keep returning the unfiltered rungs: the broker computes
  // `busy` from them to tell a "wait, the slot is taken" refusal apart from "nothing can serve
  // this", and a delay-filtered view there would turn a wait into a hard refusal.
  const fallbackDelays = CONFIG.profileFallbackAfterMs?.[profile] ?? [];
  const elapsed = Number.isFinite(Number(waitedMs)) && Number(waitedMs) >= 0 ? Number(waitedMs) : 0;
  const selectableFallbackGroups = fallbackGroups.filter((_, index) => (fallbackDelays[index] ?? 0) <= elapsed);
  const availableFallback = selectableFallbackGroups
  ...
  if (!available.length) {
    ...
    // ☠️ ONLY when context is what disqualified everything. The guard above this line used to be
    // the whole test, and "nothing is available" is also true when every target simply has its
    // slots taken -- so a lane whose primary was merely FULL fell straight through to the cloud
    // window, pre-empting any delay configured on the rung it reached. Measured in production:
    // 183 cloud leases in eight minutes on a lane configured to wait 60s first.
    // A target that FITS the request but is full is a wait: the broker turns this null into
    // `target-busy`, the caller retries, and the rung still opens once its delay elapses. Only
    // when nothing can hold the request even with every slot free is a refusal unrecoverable,
    // and that is the case this rescue exists for.
    const capacityBlocked = [...primaryCandidates, ...fallbackGroups.flat().map((id) => targetForID(id, targets))]
      .some((target) => target && targetFull(target, active) && eligible(target, { ...fit, ignoreCapacity: true }));
    if (capacityBlocked) return null;
