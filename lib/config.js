...
// `profileFallbackAfterMs` declares, per profile, how long a request must already have
// waited before each fallback group becomes eligible -- so a rung is overflow AFTER waiting
// rather than an alternative TO it. The delay array is positional against the AUTHORED group
// array, and ☠️ the delays are zipped through the SAME LAN filter that deletes emptied groups
// below. That deletion shifts every later index, so a delay re-read by authored position would
// silently attach to the wrong rung; carrying each delay with its group through the filter is
// the only safe spelling. Returns aligned `{ groups, delays }`.
const normalizeProfileFallbacks = (value, delayValue, known) => {
  // The delay map is validated once, as a whole: it must be a plain object keyed by profile.
  // Anything else (bare number, string, array, null) is ignored entirely so every profile keeps
  // immediate fallback -- the safe default -- with one error.
  const delayMap = delayValue && typeof delayValue === "object" && !Array.isArray(delayValue)
    ? delayValue
    : null;
  if (delayValue !== undefined && delayMap === null) {
    console.error(`opencode-broker: profileFallbackAfterMs must be an object mapping each profile to an array of milliseconds -- ignored, every profile falls back immediately.`);
  }
  // A delay entry naming something that is not a routing profile is ignored, one error each,
  // matching the profileCloudEgress convention above.
  if (delayMap) {
    for (const profile of Object.keys(delayMap)) {
      if (!PROFILE_NAMES.includes(profile)) {
        console.error(`opencode-broker: profileFallbackAfterMs names "${profile}", which is not a routing profile -- ignored.`);
      }
    }
  }
  ...
  for (const profile of PROFILE_NAMES) {
    ...
    const authored = delayMap ? delayMap[profile] : undefined;
    // A per-profile delay value that is present but not an array is ignored entirely: the
    // profile keeps immediate fallback (every group defaults to 0), one error.
    let authoredDelays = null;
    if (authored !== undefined) {
      if (Array.isArray(authored)) authoredDelays = authored;
      else console.error(`opencode-broker: profileFallbackAfterMs.${profile} must be an array of milliseconds -- ignored, ${profile} falls back immediately.`);
    }
    // Zip each authored group with its delay BEFORE the LAN filter, so the delay travels with
    // its group. A missing entry (short array) defaults to 0 silently -- that is the documented
    // shorthand, not a mistake. A non-finite or negative value is coerced to 0, one error each.
    const pairs = groups.map((group, index) => {
      const raw = authoredDelays ? authoredDelays[index] : undefined;
      let delay = 0;
      if (raw !== undefined) {
        if (typeof raw === "number" && Number.isFinite(raw) && raw >= 0) delay = raw;
        else console.error(`opencode-broker: profileFallbackAfterMs.${profile}[${index}] = ${JSON.stringify(raw)} is not a non-negative finite number of milliseconds -- using 0.`);
      }
      return { group: idList(group, known), delay };
    });
    // Surplus delay entries (past the authored group count) name a rung that does not exist;
    // ignore each with its own error so a second bad value is not hidden behind a summary.
    if (authoredDelays) {
      for (let index = groups.length; index < authoredDelays.length; index += 1) {
        console.error(`opencode-broker: profileFallbackAfterMs.${profile}[${index}] = ${JSON.stringify(authoredDelays[index])} has no matching fallback group -- ignored.`);
      }
    }
    const kept = pairs
      .map(({ group, delay }) => {
        if (mayLeaveLan) return { group, delay };
        const lan = group.filter((id) => known[id]?.kind === "local");
        if (lan.length === group.length) return { group, delay };
        ...
        return { group: Object.freeze(lan), delay };
      })
      .filter(({ group }) => group.length);
    fallbacks[profile] = Object.freeze(kept.map(({ group }) => group));
    delays[profile] = Object.freeze(kept.map(({ delay }) => delay));
  }
  return Object.freeze({
    groups: Object.freeze(fallbacks),
...
// One normalization, two aligned public keys: `profileFallbacks` keeps the array-of-groups
// shape every routing consumer already reads, and `profileFallbackAfterMs` is the parallel
// per-profile delay array. They are index-aligned by construction (see normalizeProfileFallbacks).
...
export const CONFIG = Object.freeze({
  ...
  // Per-profile, per-rung wait thresholds in ms, index-aligned with profileFallbacks above.
  // A profile with no entry (or an all-zero array) falls back immediately, exactly as before.
